...
class ATESeekBarPreference @JvmOverloads constructor(
    ...
    init {
        val attributes =
            context.obtainStyledAttributes(attrs, allen.town.podcast.theme.R.styleable.ATESeekBarPreference, 0, 0)

        attributes.getString(allen.town.podcast.theme.R.styleable.ATESeekBarPreference_ateKey_pref_unit)?.let {
            unit = it
        }
        attributes.recycle()
